...
import 'package:dream_gallary/screen/preorder_list/preorder_details.dart';
...
class _PreOrderState extends State<PreOrder> {
  ...
  Future<void> _getAllOrder() async {
    var res = await CallApi().getData('/app/pre-order/order');
    ...
    setState(() {
      store.state.preOrderDataState = body['order']['data'];
      store.dispatch(PreOrderDataAction(store.state.preOrderDataState));
    });
    setState(() {
        ...
        SingleChildScrollView(
          ...
          child: (activeMenu == 0)
              ? Column(
                  ...
                  children: [
                    ...List.generate(
                      store.state.preOrderDataState.length,
                      (index) {
                        return GestureDetector(
                            onTap: () {
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => PreOrderDetails(
                                            orderId: store.state
                                                .preOrderDataState[index]['id'],
                                            name: store.state
                                                    .preOrderDataState[index]
                                                ['name'],
                                            contact: store.state
                                                    .preOrderDataState[index]
                                                ['contact'],
                                            date: store.state
                                                    .preOrderDataState[index]
                                                ['created_at'],
                                            paymentStatus: store.state
                                                    .preOrderDataState[index]
                                                ['paymentStatus'],
                                            paymentMethod: store.state
                                                    .preOrderDataState[index]
                                                ['paymentType'],
                                            billingAddress: store.state
                                                    .preOrderDataState[index]
                                                ['billingAddress'],
                                            billingCity: store.state
                                                    .preOrderDataState[index]
                                                ['billingCity'],
                                            discount: store.state
                                                    .preOrderDataState[index]
                                                ['discount'],
                                            deliveryCharge: store.state
                                                    .preOrderDataState[index]
                                                ['shippingPrice'],
                                            ...
                                            subTotal: store.state
                                                    .preOrderDataState[index]
                                                ['subTotal'],
                                            orderDetails: store.state
                                                    .preOrderDataState[index]
                                                ['orderdetails'],
                                            status: store.state
                                                    .preOrderDataState[index]
                                                ['status'],
                                            giftVoucherCode: store.state
                                                    .preOrderDataState[index]
                                                ['giftVoucherCode'],
                                            referralCode: store.state
                                                    .preOrderDataState[index]
                                                ['referralCode'],
                                            coupon: store.state
                                                    .preOrderDataState[index]
                                                ['coupon'],
                                            discountType: store.state
                                                    .preOrderDataState[index]
                                                ['discountType'],
                                            giftVoucherAmount: store.state
                                                    .preOrderDataState[index]
                                                ['giftVoucherAmount'],
                                          ))); //order:store.state.preOrderDataState,
                            },
                            child: Container(
                                ...
                                child: MyOrderCard(
                                  orderId: store.state.preOrderDataState[index]
                                      ['id'],
                                  name: store.state.preOrderDataState[index]
                                      ['name'],
                                  phone: store.state.preOrderDataState[index]
                                      ['contact'],
                                  date: store.state.preOrderDataState[index]
                                      ['created_at'],
                                  paymentMethod: store.state
                                      .preOrderDataState[index]['paymentType'],
                                  payment: store.state.preOrderDataState[index]
                                      ['paymentStatus'],
                                  amount: store.state.preOrderDataState[index]
                                          ['grandTotal'] +
                                      store.state.preOrderDataState[index]
                                          ['shippingPrice'],
                                  status: store.state.preOrderDataState[index]
                                      ['status'],
                                  index: index,
                                )));
  ...
  Future<void> _egerLoadingMyOrder({limit}) async {
    var resAllMyOrder =
        await CallApi().getData("/app/pre-order/order?isApp=1&limit=$limit");
    ...
    if (resAllMyOrder.statusCode == 200) {
      ...
      print(body['order']);
      setState(() {
        store.state.preOrderDataState = body['order'];
        store.dispatch(MyOrderAction(store.state.preOrderDataState));
      });
    }
    setState(() {
      store.dispatch(IsLoadingAction(false));
  ...
  Future<void> statusApi() async {
    var resAllStatusData = await CallApi()
        .getData("/app/pre-order/order?isApp=1&status=$selectedValue");
